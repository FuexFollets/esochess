...
namespace esochess {
    ...
    void add_bishop_moves(bitboard& board, bitboard::moves_listing& moves_listing_ext, const bitboard::cordinate& at_cordinate) {
        const bitboard::Turn turn {board.turn()};
        const bitboard::Turn opponents_turn {bitboard::opposite_turn(turn)};
        const bitboard::bit_representation bishop_cordinate_bits {at_cordinate.to_bit_representation()};
        ...
        for (const bitboard::Direction& direction: bitboard::pieces::bishop_directions) {
            for (int steps {}; steps < 8; steps++) {
                ...
                if ((opponent_piece_positions_bits & cordinate_after_steps_bits) != 0) {
                    moves_listing_ext.normal_moves.emplace_back(bishop_cordinate_bits, cordinate_after_steps_bits);
    ...
    void add_rook_moves(bitboard& board, bitboard::moves_listing& moves_listing_ext, const bitboard::cordinate& at_cordinate) {
        const bitboard::Turn turn {board.turn()};
        const bitboard::Turn opponents_turn {bitboard::opposite_turn(turn)};
        const bitboard::bit_representation rook_cordinate_bits {at_cordinate.to_bit_representation()};
        const bitboard::bit_representation current_turns_pieces_positions_bits {board.bitboard_bitor_accumulation(turn)};
        const bitboard::bit_representation opponent_piece_positions_bits {board.bitboard_bitor_accumulation(opponents_turn)};
        ...
        for (const bitboard::Direction& direction: bitboard::pieces::rook_directions) {
            for (int steps {}; steps < 8; steps++) {
                ...
                if ((opponent_piece_positions_bits & cordinate_after_steps_bits) != 0) {
                    moves_listing_ext.normal_moves.emplace_back(rook_cordinate_bits, cordinate_after_steps_bits);
